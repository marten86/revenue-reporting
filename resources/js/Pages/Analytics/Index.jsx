        ...
        <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:8,padding:'10px 14px',boxShadow:'0 4px 12px rgba(0,0,0,.1)'}}>
            ...
            {payload.map((p, i) => (
                <p key={i} style={{color:p.color,fontSize:12,margin:'2px 0'}}>
                    {p.name}: {typeof p.value === 'number' && p.name !== 'Growth %'
            ...
            <div style={{padding:'24px 32px',maxWidth:1400,margin:'0 auto'}}>
                ...
                <div style={{display:'flex',gap:16,marginBottom:24,flexWrap:'wrap'}}>
                    ...
                    <SummaryCard title="Capaian" icon="📊" color={achievementColor}
                        ...
                        sub={summary.achievement >= 100
                            ? '✅ Target tercapai'
                            : `Kurang ${formatRp(summary.target - summary.total_revenue)}`} />
                    <SummaryCard title="Growth vs Periode Lalu" icon="📉"
                        color={summary.growth !== null ? growthColor : '#6b7280'}
                        value={summary.growth !== null
                            ? `${summary.growth > 0 ? '+' : ''}${summary.growth}%`
                            : '—'}
                        sub={summary.growth !== null
                            ? (summary.growth >= 0 ? 'Naik dari periode lalu' : 'Turun dari periode lalu')
                            : 'Data tidak cukup'} />
                </div>
                    ...
                    <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:12,padding:20}}>
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <ComposedChart data={chartMain || []} margin={{top:5,right:showGrowth?50:20,left:10,bottom:5}}>
                                ...
                                <Bar yAxisId="left" dataKey="actual" name="Aktual"
                                    fill="#16a34a" radius={[4,4,0,0]} maxBarSize={40} />
                                <Line yAxisId="left" dataKey="target" name="Target"
                                    stroke="#ef4444" strokeWidth={2} dot={false} strokeDasharray="5 5" />
                                {showGrowth && (
                                    <Line yAxisId="right" dataKey="growth" name="Growth %"
                        ...
                        {(byChannel || []).length > 0 ? (
                            <>
                                <ResponsiveContainer width="100%" height={180}>
                                    <PieChart>
                                        <Pie data={byChannel} dataKey="total" nameKey="channel"
                                    ...
                                    {byChannel.map((c, i) => (
                                        <div key={i} style={{display:'flex',alignItems:'center',gap:8,fontSize:12}}>
                                            <div style={{width:10,height:10,borderRadius:'50%',flexShrink:0,
                                                background:CHANNEL_COLORS[c.channel]||COLORS[i%COLORS.length]}} />
                ...
                <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:20,marginBottom:20}}>
                    ...
                    <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:12,padding:20}}>
                        ...
                        {(byBranch || []).length > 0 ? (
                            ...
                            <div style={{textAlign:'center',color:'#9ca3af',paddingTop:80}}>
                                Pilih "Semua Cabang" untuk melihat perbandingan
                            </div>
                        )}
                    </div>
                    ...
                    <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:12,padding:20}}>
                        ...
                        {(byBranch || []).length > 0 ? (
                            ...
                            <div style={{textAlign:'center',color:'#9ca3af',paddingTop:80}}>
                                Pilih "Semua Cabang" untuk melihat capaian
                            </div>
                        )}
                    </div>
                </div>
                            ...
                            <thead>
                                <tr style={{background:'#f9fafb'}}>
                                    <th style={{padding:'8px 12px',textAlign:'left',color:'#6b7280',fontWeight:600,
                                        borderBottom:'1px solid #e5e7eb',whiteSpace:'nowrap'}}>Cabang</th>
                                    {MONTH_SHORT.map(m => (
                                        <th key={m} style={{padding:'8px 6px',textAlign:'right',color:'#6b7280',
                                            fontWeight:600,borderBottom:'1px solid #e5e7eb',minWidth:72}}>{m}</th>
                                    ))}
                                    <th style={{padding:'8px 12px',textAlign:'right',color:'#6b7280',
                                        fontWeight:600,borderBottom:'1px solid #e5e7eb',whiteSpace:'nowrap'}}>Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr key={ri} style={{borderBottom:'1px solid #f3f4f6'}}
                                        ...
                                        {(row.months || []).map((m, mi) => (
                                            <td key={mi} style={{padding:'6px 6px',textAlign:'right'}}>
                                                <div style={{color:m.actual>0?'#111827':'#d1d5db'}}>
                                                    {formatRp(m.actual)}
                                                </div>
                                                {m.target > 0 && (
                                                    <div style={{fontSize:10,
                                                        color:m.pct>=100?'#16a34a':m.pct>=75?'#d97706':'#dc2626'}}>
                                                        {m.pct}%
                                                    </div>
                                                )}
                                            </td>
